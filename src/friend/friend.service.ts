...
import { FRIEND_TYPE } from 'src/enum/friend.enum';
...
export class FriendService {
  ...
  async acceptFriendRequest(user_id: string, target_id: string) {
    try {
      const friendType = await this.checkFriendType(user_id, target_id);
      console.log('FriendService ~ friendType:', friendType);
  ...
  async sendFriendRequest(user_id: string, target_id: string) {
    try {
      ...
      console.log('FriendService ~ sendFriendRequest ~ error:', error);
      return new BaseResponse(400, 'FAIL', error);
    }
  }
  ...
  async removeFriend(user_id: string, target_user_id: string) {
  ...
  async checkFriendType(user_id: string, target_id: string) {
    console.log('FriendService ~ checkFriendType ~ target_id:', target_id);
    console.log('FriendService ~ checkFriendType ~ user_id:', user_id);
